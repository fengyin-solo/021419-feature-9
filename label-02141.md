...
1. **插入破坏内容**：`insertText('![alt](', ')')` 把选中文本塞进 URL 位、alt 写死为 "alt"；光标在已有图片语法上再点插入会嵌套出 `![alt](![alt](…))` 破坏正文
2. **无校验**：无替代文本/地址校验；`![]()` 空地址解析器不识别；地址含空格或圆括号会生成破碎语法
3. **状态不可恢复**：无加载态、无超时、无重试；且旧实现用 `innerHTML` 拼接 url/alt，存在 XSS 隐患
4. **误判**：`data:`/`blob:` 落入"其他"占位符；同源 `/images/x.png` 被误判为本地路径直接拒绝；未知协议（含 `javascript:`）会被尝试加载
5. **保存提示不同步**：mounted 时 `updateContent` 导致一打开就显示未保存；`markSaved` 无任何调用方

#### 实现方案

1. **新增 `src/editor/image-url.js`**：地址分类（empty/network/embed/local/relative/unknown-protocol）与校验（error 阻止提交、warning 允许提交）纯函数，判定只影响预览层，绝不改动正文
2. **重写 ImageWidget 状态机**：loading → loaded / error / timeout（15s 超时中断请求）；empty / local / unknown-protocol 提示态；失败与超时提供「重试」按钮，重试复用构造时固定的 `this.url`——中断后重试仍对应同一张图片；`attempt` 序号使过期异步回调失效；全部用 `createElement`/`textContent` 构建 DOM，消除 XSS
3. **新增 `ImageDialog.vue`**：插入/编辑对话框，实时校验；选中文本保留为替代文本；光标在已有图片上时进入编辑模式原位替换（保留原 title），杜绝嵌套；对话框单例，重复点击不重复插入
4. **保存提示同步**：初始加载后 `markSaved()` 修复误标；地址修改 → `eq` 判定失败 → 新 widget 加载新地址（预览同步）+ `isDirty` 更新（保存提示同步）；新增 Ctrl/Cmd+S 标记已保存
5. **解析器**：图片正则支持空地址（空态）与可选 title

#### 生成的代码

##### `src/editor/image-url.js`（新增）
...
##### `src/editor/decoration-plugin.js` — ImageWidget 重写（核心片段）
...
class ImageWidget extends WidgetType {
  ...
  renderRetryable(wrapper, reason) {
    // …图标 + 标题 + 地址 + 操作提示 + 「重试」按钮…
    // 按钮 click → this.startLoad(wrapper)（同一 this.url）
  }
...
##### `src/editor/markdown-parser.js` — 图片正则
...
##### `src/components/ImageDialog.vue`（新增，核心逻辑）
...
// error 级禁止提交（会生成破碎语法或必然无效）；warning 级允许提交
...
##### `src/App.vue` — 插入/编辑接线（核心片段）

```javascript
// 对话框为单例：重复点击工具栏按钮不会重复打开，也不会重复插入
const imageDialog = reactive({ visible: false, mode: 'insert', alt: '', url: '', title: '', from: 0, to: 0 })
...
function openImageDialog() {
  ...
  if (region) {
    Object.assign(imageDialog, {
      ...
      alt: region.meta.alt, url: region.meta.url,
      title: region.meta.title || '', // 保留原 title，替换时原样写回
      from: region.from, to: region.to
    })
  } else {
    // 选中文本保留为替代文本，不会被丢弃
    const sel = state.sliceDoc(from, to)
    Object.assign(imageDialog, { visible: true, mode: 'insert', alt: sel, url: '', title: '', from, to })
  }
}
...
function confirmImage({ alt, url }) {
  ...
  const from = Math.min(imageDialog.from, docLen) // 边界保护
  const to = Math.min(imageDialog.to, docLen)
  const title = imageDialog.title ? ` ${imageDialog.title}` : ''
  const text = `![${alt}](${url}${title})`
  editorView.dispatch({
    changes: { from, to, insert: text },
    selection: { anchor: from + text.length } // 光标移出该行 → 立即渲染新预览
  })
  editorView.focus()
}

// Ctrl/Cmd+S：标记已保存（保存提示与正文状态同步）
...
##### `src/components/EditorPane.vue` — 修复初始保存态
...
##### `src/styles/editor-theme.scss` — 图片状态样式（新增）

```scss
.md-image-box { display: block; margin: $sp-2 0; }

.md-image-box--loading .md-image-status {
  display: flex; align-items: center; gap: $sp-2;
  background: $bg-code; border: 1.5px dashed $border;
  border-radius: $r-md; padding: $sp-4 $sp-5;
  color: $text-3; font-size: $fs-sm; min-height: 72px;
}
...
.md-image-spinner {
  ...
  border-radius: 50%;
  animation: md-image-spin 0.8s linear infinite;
}
@keyframes md-image-spin { to { transform: rotate(360deg); } }

.md-image-notice {
  display: flex; flex-direction: column; align-items: center; gap: $sp-2;
  background: $bg-code; border: 1.5px dashed $border; border-radius: $r-md;
  padding: $sp-4 $sp-5; color: $text-3; font-size: $fs-sm;
  text-align: center; line-height: 1.6;
}
.md-image-box--local .md-image-notice,
.md-image-box--unknown-protocol .md-image-notice {
  border-color: rgba($warning, 0.45);
  .md-image-icon, .md-image-notice-title { color: $warning; }
}
.md-image-box--error .md-image-notice,
.md-image-box--timeout .md-image-notice {
  border-color: rgba($error, 0.4);
  .md-image-icon, .md-image-notice-title { color: $error; }
}

.md-image-action {
  margin-top: $sp-1; padding: 3px $sp-4;
  border: 1px solid $border; border-radius: $r-full;
  background: $bg-elevated; font-family: $font-ui; font-size: $fs-xs;
  color: $text-2; cursor: pointer; transition: all $t-fast $ease;
  &:hover { border-color: $accent; color: $accent; background: $accent-soft; }
...
| 需求 | 实现 |
|------|------|
| 选中文本/光标插入保留原内容 | 选中文本自动成为替代文本；光标处纯插入 |
| 替代文本和地址校验 | 对话框实时校验，error 阻止提交，warning 可提交 |
| 加载/空态/失败/重试可恢复 | loading spinner、empty 提示、error/timeout + 重试按钮 |
| 未知地址不破坏正文 | relative 尝试加载，失败进可重试错误态，正文不动 |
| 未知协议不破坏正文 | unknown-protocol 提示态，不发起请求（含安全考虑） |
| 网络超时不破坏正文 | 15s 超时中断请求 → 超时态可重试 |
| 重复插入不破坏正文 | 对话框单例；光标在图片上时转为编辑模式，不嵌套 |
| 本地路径误判不破坏正文 | 精确识别 file://、盘符、UNC、~；/、./、裸文件名改为尝试加载，不再误判 |
| 地址修改后三处同步 | eq 比对 url → 新 widget 加载（预览）；updateContent → isDirty（保存提示）；初始 markSaved 修复 |
| 中断后重试同一张图 | 重试复用构造时固定的 this.url，attempt 序号防止旧回调串扰 |

#### 验证结果

- `npm run build` ✅ 构建成功
- 地址分类与解析器单元测试（18 个分类用例 + 校验 + 空地址/title/嵌套解析）✅ 全部通过
- `vite preview` 冒烟测试 ✅ 页面正常返回
- README.md 图片章节已同步更新
